...
# caracteres especiais — títulos de novel, os emojis do log (★ ⏳ ✓), URLs com
# acento, etc. O resultado é um UnicodeEncodeError durante o logging que, por
# acontecer dentro do bloco de request, era confundido com falha de rede e
# disparava retries inúteis. Forçar UTF-8 no stdout/stderr resolve na raiz.
# errors="replace" é um seguro extra: um caractere impossível vira '?' em vez
# de derrubar o script. reconfigure() existe desde o Python 3.7.
...
def setup_logging(verbose: bool, log_file: Optional[str]) -> None:
    ...
    # Blindagem extra: se um caractere ainda escapar do reconfigure UTF-8 acima
    # (console muito antigo, stream redirecionado de forma estranha), o handler
    # troca o caractere problemático em vez de lançar UnicodeEncodeError — que
    # antes era erroneamente tratado como falha de rede pelo retry wrapper.
